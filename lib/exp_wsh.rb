...
# WMI
show wmi = WIN32OLE.connect("winmgmts:"), 'wmi'

DRIVE_TYPE = 3
drives = wmi.ExecQuery "select DeviceId from Win32_LogicalDisk"# where DriveType = #{DRIVE_TYPE}"
show drives, 'Drives collection'
drives.each {|drive| p drive.DeviceID; show drive, 'Drive'}
p drives.count
